...
export const FileUpload = ({
  ...
  return <Card className="p-8 border-2 border-dashed border-muted-foreground/30 hover:border-primary/50 transition-colors duration-300 bg-gray-200">
      <div {...getRootProps()} className={`cursor-pointer text-center transition-colors duration-300 ${isDragActive ? 'text-primary' : 'text-muted-foreground'}`}>
        <input {...getInputProps()} />
        <div className="flex flex-col items-center space-y-4">
          <div className={`p-4 rounded-full transition-colors duration-300 ${isDragActive ? 'bg-primary/10' : 'bg-muted/30'}`}>
            {isDragActive ? <Upload className="h-8 w-8 text-primary" /> : <FileSpreadsheet className="h-8 w-8" />}
          </div>
          <div>
